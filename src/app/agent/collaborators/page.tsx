...
type CollaboratorStatus = 'pending' | 'approved' | 'rejected';
type Collaborator = { id: string; name: string; email: string; phone: string; code: string; totalCommission: number; orders: number; revenue: number; status: CollaboratorStatus };
...
export default function AgentCollaboratorsPage() {
    ...
    return <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="flex items-center gap-3 text-2xl font-bold text-slate-900"><span className="grid h-11 w-11 place-items-center rounded-xl bg-[#F5EFE6] text-[#9C7044]"><Users /></span>Cộng tác viên</h1><p className="mt-1 text-sm text-slate-500">Tạo tài khoản và theo dõi cả hồ sơ CTV đang chờ Admin duyệt.</p></div><button onClick={() => setModal(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#9C7044] px-5 py-3 font-bold text-white hover:bg-[#7d5a36]"><UserPlus size={19} /> Tạo cộng tác viên</button></div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="CTV đã duyệt" value={String(approvedItems.length)} /><Metric label="Chờ Admin duyệt" value={String(pendingCount)} /><Metric label="Đơn hợp lệ" value={String(approvedItems.reduce((sum, item) => sum + item.orders, 0))} /><Metric label="Doanh thu" value={money(approvedItems.reduce((sum, item) => sum + item.revenue, 0))} /></div>
        ...
        {loading ? <div className="grid min-h-64 place-items-center"><Loader2 className="animate-spin text-[#9C7044]" /></div> : error ? <div className="grid min-h-64 place-items-center text-red-600">{error}</div> : filtered.length === 0 ? <div className="grid min-h-64 place-items-center p-6 text-center text-slate-500"><div><Users className="mx-auto mb-3 text-slate-300" size={38} /><p className="font-semibold text-slate-700">Chưa có cộng tác viên hoặc hồ sơ đăng ký</p><button onClick={() => setModal(true)} className="mt-4 font-semibold text-[#9C7044] hover:underline">Tạo cộng tác viên đầu tiên</button></div></div> : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-5 py-4">Cộng tác viên</th><th className="px-5 py-4">Mã / link</th><th className="px-5 py-4">Đơn</th><th className="px-5 py-4">Doanh thu</th><th className="px-5 py-4">Hoa hồng</th><th className="px-5 py-4">Thao tác</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((item) => <tr key={item.id} className="hover:bg-slate-50"><td className="px-5 py-4"><div className="flex items-center gap-2"><span className="font-semibold text-slate-900">{item.name}</span><StatusBadge status={item.status} /></div><div className="text-xs text-slate-500">{item.email}</div></td><td className="px-5 py-4">{item.code ? <button onClick={() => void copyLink(item)} className="inline-flex items-center gap-2 rounded-lg bg-slate-100 px-3 py-2 font-mono font-semibold text-slate-700 hover:bg-slate-200">{item.code} {copiedId === item.id ? <Check size={15} className="text-emerald-600" /> : <Copy size={15} />}</button> : <span className="text-xs font-medium text-amber-700">Cấp sau khi Admin duyệt</span>}</td><td className="px-5 py-4">{item.orders}</td><td className="px-5 py-4 font-semibold">{money(item.revenue)}</td><td className="px-5 py-4 font-semibold text-emerald-600">{money(item.totalCommission)}</td><td className="px-5 py-4">{item.status === 'approved' ? <button onClick={() => void disable(item)} className="rounded-lg p-2 text-red-500 hover:bg-red-50" aria-label={`Vô hiệu hóa ${item.name}`}><Trash2 size={18} /></button> : <span className="text-xs text-slate-500">{item.status === 'pending' ? 'Admin đang xét duyệt' : 'Hồ sơ đã bị từ chối'}</span>}</td></tr>)}</tbody></table></div>}</section>
...
function StatusBadge({ status }: { status: CollaboratorStatus }) {
    ...
    return <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${styles}`}>{label}</span>;
}
