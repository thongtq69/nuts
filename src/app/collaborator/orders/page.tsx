...
export default function CollaboratorOrdersPage() {
    ...
    const viewOrderDetail = async (order: Order) => {
        ...
        try {
            const res = await fetch(`/api/collaborator/orders/${order._id}`);
            if (res.ok) {
                const data = await res.json();
                setOrderDetail(data);
            }
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {loading ? (
                                ...
                                filteredOrders.map((order) => {
                                    const status = statusConfig[order.status] || { label: order.status, class: 'bg-gray-100 text-gray-700', color: '#6b7280' };
                                    return (
                                        <tr key={order._id} className="hover:bg-amber-50/30 transition-colors">
                                            ...
                                            <td className="px-6 py-5 text-center">
                                                <span className={`inline-flex px-3 py-1.5 rounded-full text-xs font-semibold ${status.class}`}>
                                                    {status.label}
                                                </span>
                                            </td>
                                            <td className="px-6 py-5 text-center">
