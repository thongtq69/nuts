...
import { ArrowDownToLine, ArrowUpFromLine, Banknote, CheckCircle2, Clock3, Coins, Gift, History, RefreshCw, ShieldCheck, Smile, Sparkles, Star, WalletCards, X } from 'lucide-react';
...
const pastelWheelColor = (color: string) => {
    const normalized = color.trim().replace('#', '');
    if (!/^[\da-f]{6}$/i.test(normalized)) return color;
    const pastelBase = [255, 255, 255];
    const source = [0, 2, 4].map(offset => Number.parseInt(normalized.slice(offset, offset + 2), 16));
    return `rgb(${source.map((channel, index) => Math.round(channel * .52 + pastelBase[index] * .48)).join(',')})`;
};
...
export default function LuckyWheelPage() {
    ...
    const wheelBackground = useMemo(() => {
        ...
        return `conic-gradient(${segments.map((segment, index) => `${pastelWheelColor(segment.color)} ${index * angle}deg ${index * angle + angle}deg`).join(',')})`;
    }, [segments]);
    ...
    return <main className="min-h-screen overflow-x-hidden bg-[linear-gradient(180deg,#f4fbff_0%,#fff9e9_48%,#f3fff8_100%)]"><Header /><Navbar /><Breadcrumb items={[{ label: 'Trang chủ', href: '/' }, { label: 'Bánh xe quà tặng' }]} />
        <section className="relative overflow-hidden pb-16 pt-5 sm:pb-24 sm:pt-8"><div className="pointer-events-none absolute inset-x-0 top-0 h-96 bg-[radial-gradient(circle_at_50%_0%,rgba(255,219,107,.38),transparent_68%)]" /><div className="pointer-events-none absolute -left-24 top-64 h-72 w-72 rounded-full bg-sky-200/35 blur-3xl" /><div className="pointer-events-none absolute -right-28 top-80 h-80 w-80 rounded-full bg-emerald-100/55 blur-3xl" /><div className="relative mx-auto max-w-7xl px-3.5 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-5xl text-center"><div className="inline-flex max-w-full items-center gap-2 rounded-full border-2 border-[#ffd66b] bg-[#fff9d9] px-3.5 py-2 text-[10px] font-extrabold uppercase tracking-[.16em] text-[#76521f] shadow-[0_5px_0_#f4c957] sm:px-4 sm:text-xs sm:tracking-[.2em]"><Star className="shrink-0 fill-[#ffd75e] text-[#e5a91d]" size={15}/> <span className="truncate">{displayMemberBadge}</span></div><h1 className={`mt-5 font-black leading-[1.08] tracking-tight text-[#28415f] sm:mt-6 ${displayCampaignName === 'Bánh xe quà tặng GO NUTS' ? 'whitespace-nowrap text-[clamp(1rem,4vw,3.4rem)]' : 'text-[2rem] sm:text-5xl lg:text-[3.4rem]'}`}>{displayCampaignName}</h1><p className="mx-auto mt-3 max-w-2xl text-[15px] leading-6 text-[#58708a] sm:mt-4 sm:text-base sm:leading-7">{displayIntro}</p></div>
            ...
            <div className="mt-8 grid items-start gap-5 sm:mt-10 sm:gap-7 lg:grid-cols-[minmax(0,1.08fr)_minmax(340px,.92fr)]">
                <div className="relative overflow-hidden rounded-[30px] border-2 border-white bg-[linear-gradient(155deg,#dff6ff,#f1fbff_48%,#fff5cd)] p-4 shadow-[0_20px_55px_rgba(65,137,174,.2)] sm:rounded-[40px] sm:p-9 lg:sticky lg:top-5"><div className="absolute -left-8 top-8 h-20 w-32 rounded-full bg-white/75" /><div className="absolute -right-8 bottom-10 h-24 w-40 rounded-full bg-white/65" /><Sparkles className="absolute right-7 top-6 text-[#f0b62d]" size={27}/><Star className="absolute bottom-8 left-8 fill-[#ffcf68] text-[#efa926]" size={24}/><div className="relative mx-auto aspect-square max-w-[530px]"><div className="absolute left-1/2 top-[-3px] z-30 -translate-x-1/2"><div className="h-0 w-0 border-l-[14px] border-r-[14px] border-t-[30px] border-l-transparent border-r-transparent border-t-[#ff7f72] drop-shadow-md sm:border-l-[18px] sm:border-r-[18px] sm:border-t-[38px]" /></div><div className="absolute inset-1 rounded-full bg-[#7fd4d0] shadow-[0_0_0_4px_#fff,0_0_0_9px_#56b8bf,0_16px_32px_rgba(54,127,151,.24)] sm:shadow-[0_0_0_6px_#fff,0_0_0_11px_#56b8bf,0_18px_38px_rgba(54,127,151,.25)]" /><div className="absolute inset-[11px] overflow-hidden rounded-full transition-transform duration-[3400ms] ease-[cubic-bezier(.12,.67,.12,1)] sm:inset-4" style={{ transform: `rotate(${rotation}deg)`, background: `radial-gradient(circle at 48% 38%, rgba(255,255,255,.34), transparent 60%), ${wheelBackground}` }}><div className="absolute inset-2 rounded-full border-2 border-white/75 shadow-[inset_0_0_24px_rgba(64,127,145,.1)] sm:inset-3" />{segments.map((segment, index) => {
                    const segmentAngle = 360 / segments.length;
                    ...
                    const readableLabelRotation = labelRotation > 90 && labelRotation < 270 ? labelRotation + 180 : labelRotation;
                    return <div data-wheel-prize key={segment.label} className="absolute z-10 flex w-[31%] items-center justify-center text-center text-[clamp(9px,2.7vw,13px)] font-extrabold uppercase leading-[1.08] tracking-[.025em] text-[#262422] [text-shadow:0_1px_0_rgba(255,255,255,.55)] sm:text-[clamp(13px,1.65vw,17px)]" style={{ left: `${50 + Math.sin(angle) * 32.5}%`, top: `${50 - Math.cos(angle) * 32.5}%`, transform: `translate(-50%, -50%) rotate(${readableLabelRotation}deg)` }}><span>{segment.label}</span></div>;
                })}</div><button onClick={spin} disabled={spinning || (!data?.campaign.adminTestMode && (!data?.campaign.active || !data?.account.availableSpins))} className="absolute left-1/2 top-1/2 z-20 grid h-[78px] w-[78px] -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border-[5px] border-white bg-[linear-gradient(145deg,#ff9b82,#ff7068)] px-2 text-center text-[11px] font-black uppercase leading-tight text-white shadow-[0_9px_0_#dc5d5a,0_14px_24px_rgba(73,91,115,.24),inset_0_2px_2px_rgba(255,255,255,.35)] transition hover:-translate-y-[52%] hover:scale-105 active:translate-y-[calc(-50%+3px)] active:shadow-[0_5px_0_#dc5d5a] disabled:cursor-not-allowed disabled:grayscale sm:h-28 sm:w-28 sm:border-[6px] sm:text-base"><span>{spinning ? 'ĐANG QUAY' : data?.campaign.spinButtonText}</span></button></div>{!data?.campaign.adminTestMode && <p className="relative mt-6 text-center text-sm font-bold text-[#42677c] sm:mt-8"><Smile className="mr-1 inline text-[#f0aa25]" size={19}/> Bạn còn <strong className="text-xl text-[#176f87]">{data?.account.availableSpins || 0}</strong> lượt vui</p>}</div>
