...
export default function LuckyWheelPage() {
    ...
    if (!user) return <main><Header /><Navbar /><Breadcrumb items={[{ label: 'Trang chủ', href: '/' }, { label: 'Vòng quay may mắn' }]} /><section className="mx-auto max-w-xl px-5 py-20 text-center"><Gift className="mx-auto mb-5 text-[#9c7043]" size={52}/><h1 className="text-2xl font-black tracking-tight sm:text-3xl">Vòng quay may mắn GO NUTS</h1><p className="mt-4 text-slate-600">Vui lòng đăng ký thành viên hoặc đăng nhập để nạp lượt và tham gia.</p><Link href="/login" className="mt-7 inline-flex rounded-full bg-[#9c7043] px-7 py-3 font-bold text-white">Đăng nhập ngay</Link></section><Footer /></main>;
    ...
    return <main className="min-h-screen overflow-x-hidden bg-[#fffaf0]"><Header /><Navbar /><Breadcrumb items={[{ label: 'Trang chủ', href: '/' }, { label: 'Bánh xe quà tặng' }]} />
        <section className="relative overflow-hidden pb-16 pt-5 sm:pb-24 sm:pt-8"><div className="pointer-events-none absolute inset-x-0 top-0 h-80 bg-[radial-gradient(circle_at_50%_0%,rgba(235,191,91,.3),transparent_70%)]" /><div className="pointer-events-none absolute -right-28 top-80 h-80 w-80 rounded-full bg-amber-100/50 blur-3xl" /><div className="relative mx-auto max-w-7xl px-3.5 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-5xl text-center"><div className="inline-flex max-w-full items-center gap-2 rounded-full border-2 border-[#ffd66b] bg-[#fff9d9] px-3.5 py-2 text-[10px] font-extrabold uppercase tracking-[.16em] text-[#76521f] shadow-[0_5px_0_#f4c957] sm:px-4 sm:text-xs sm:tracking-[.2em]"><Star className="shrink-0 fill-[#ffd75e] text-[#e5a91d]" size={15}/> <span className="truncate">{displayMemberBadge}</span></div><h1 className={`mt-5 font-black leading-[1.08] tracking-tight text-[#4a3424] sm:mt-6 ${displayCampaignName === 'Bánh xe quà tặng GO NUTS' ? 'whitespace-nowrap text-[clamp(1rem,4vw,3.4rem)]' : 'text-[2rem] sm:text-5xl lg:text-[3.4rem]'}`}>{displayCampaignName}</h1><p className="mx-auto mt-3 max-w-2xl text-[15px] leading-6 text-[#765f4b] sm:mt-4 sm:text-base sm:leading-7">{displayIntro}</p></div>
