...
const labels = ['Chúc may mắn', '1.000đ', 'Chúc may mắn', '5.000đ', 'Chúc may mắn'];
...
export default function LuckyWheelPage() {
    ...
    const spin = async () => {
        ...
        const candidates = prize === 1000 ? [1] : prize === 5000 ? [3] : [0, 2, 4];
        ...
        window.setTimeout(() => {
            ...
            setMessage(prize ? `Chúc mừng! Bạn nhận voucher ${prize.toLocaleString('vi-VN')}đ.` : 'Chúc bạn may mắn ở lượt tiếp theo!');
            setSpinning(false);
        }, 3200);
    };
    ...
    if (!user) return (
        <main><Header /><Navbar /><Breadcrumb items={[{ label: 'Trang chủ', href: '/' }, { label: 'Vòng quay may mắn' }]} />
            <section className="mx-auto max-w-xl px-5 py-20 text-center"><h1 className="text-3xl font-bold">Vòng quay tri ân khách hàng</h1><p className="mt-4 text-slate-600">Bạn cần đăng ký thành viên hoặc đăng nhập để nhận và sử dụng lượt quay.</p><Link href="/login" className="mt-7 inline-block rounded-xl bg-amber-500 px-6 py-3 font-bold text-slate-900">Đăng nhập ngay</Link></section><Footer /></main>
    );

    return (
        <main className="bg-amber-50/40"><Header /><Navbar /><Breadcrumb items={[{ label: 'Trang chủ', href: '/' }, { label: 'Vòng quay may mắn' }]} />
            <section className="mx-auto max-w-6xl px-4 py-10">
                <div className="text-center"><p className="font-semibold uppercase tracking-widest text-amber-700">Chương trình tri ân</p><h1 className="mt-2 text-3xl font-black text-slate-900 sm:text-4xl">{data?.campaign?.name}</h1><p className="mx-auto mt-3 max-w-2xl text-slate-600">Mỗi đơn hàng thật đã hoàn tất từ {Number(data?.campaign?.qualifyingOrderMinimum || 10000).toLocaleString('vi-VN')}đ nhận 5 lượt. Phần thưởng chỉ là voucher mua hàng, không quy đổi thành tiền mặt.</p></div>
                ...
                <div className="mt-10 grid gap-8 lg:grid-cols-[1.1fr_.9fr]">
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="rounded-2xl border bg-white p-6"><h2 className="text-lg font-bold">Thể lệ minh bạch</h2><ul className="mt-3 space-y-2 text-sm text-slate-600"><li>• Một đơn hàng sản phẩm đã hoàn tất từ 10.000đ nhận đúng 5 lượt, mỗi đơn chỉ cấp một lần.</li><li>• Trong mỗi nhóm 5 lượt: 3 lượt chúc may mắn, 1 voucher 1.000đ và 1 voucher 5.000đ.</li><li>• Voucher có hạn 30 ngày, không chuyển nhượng, không rút tiền và không dùng để mua lượt.</li><li>• Khi doanh thu hợp lệ đạt mỗi mốc 1 tỷ đồng: hệ thống chọn 15 thành viên khác nhau để tặng 10 voucher 100.000đ và 5 voucher 50.000đ.</li></ul></div>
                        <div className="rounded-2xl border bg-white p-6"><h2 className="font-bold">Lịch sử gần đây</h2><div className="mt-3 max-h-64 space-y-2 overflow-auto">{!data?.history?.length ? <p className="text-sm text-slate-500">Chưa có lượt quay.</p> : data.history.map(item => <div key={item._id || item.requestId} className="flex justify-between rounded-lg bg-slate-50 p-3 text-sm"><span>{item.createdAt ? new Date(item.createdAt).toLocaleString('vi-VN') : ''}</span><strong className={item.prizeValue ? 'text-emerald-600' : 'text-slate-500'}>{item.prizeValue ? `Voucher ${Number(item.prizeValue).toLocaleString('vi-VN')}đ` : 'Chúc may mắn'}</strong></div>)}</div></div>
                    </div>
                </div>
            </section><Footer />
        </main>
