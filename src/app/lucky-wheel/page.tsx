...
interface Withdrawal { _id: string; amount: number; status: 'pending' | 'paid' | 'rejected'; payoutReference?: string; bankTransactionId?: string; bankVerifiedAt?: string; createdAt: string }
...
export default function LuckyWheelPage() {
    ...
    return <main className="min-h-screen bg-[#fffaf0]"><Header /><Navbar /><Breadcrumb items={[{ label: 'Trang chủ', href: '/' }, { label: 'Vòng quay may mắn' }]} />
        <section className="relative overflow-hidden pb-20 pt-7"><div className="pointer-events-none absolute inset-x-0 top-0 h-72 bg-[radial-gradient(circle_at_50%_0%,rgba(235,191,91,.26),transparent_70%)]" /><div className="relative mx-auto max-w-6xl px-4">
            ...
            {!!data?.withdrawals.length && <section className="rounded-3xl border border-[#eadcc8] bg-white p-6 lg:col-span-2"><h2 className="flex items-center gap-2 text-lg font-black"><ArrowDownToLine className="text-[#a16d35]"/> Trạng thái rút tiền</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{data.withdrawals.map(item => <div key={item._id} className="rounded-2xl bg-[#fffaf2] p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-black text-[#33271e]">{money(item.amount)}</p><p className="mt-1 text-xs text-slate-500">{new Date(item.createdAt).toLocaleString('vi-VN')} · {item.payoutReference || 'Đang xử lý'}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${item.status === 'paid' ? 'bg-emerald-100 text-emerald-700' : item.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{item.status === 'paid' ? 'Đã chuyển thành công' : item.status === 'rejected' ? 'Đã từ chối' : 'Đang chờ admin chuyển'}</span></div>{item.bankTransactionId && <p className="mt-3 text-xs font-semibold text-emerald-700">Mã giao dịch ngân hàng: {item.bankTransactionId}</p>}</div>)}</div></section>}
            </div>
        </div></section>
