    ...
    return <main className="min-h-screen overflow-x-hidden bg-[#fffaf0]"><Header /><Navbar /><Breadcrumb items={[{ label: 'Trang chủ', href: '/' }, { label: 'Vòng quay may mắn' }]} />
        <section className="relative overflow-hidden pb-16 pt-5 sm:pb-24 sm:pt-8"><div className="pointer-events-none absolute inset-x-0 top-0 h-80 bg-[radial-gradient(circle_at_50%_0%,rgba(235,191,91,.3),transparent_70%)]" /><div className="pointer-events-none absolute -right-28 top-80 h-80 w-80 rounded-full bg-emerald-100/45 blur-3xl" /><div className="relative mx-auto max-w-7xl px-3.5 sm:px-6 lg:px-8">
            ...
            <div className="mt-8 grid items-start gap-5 sm:mt-10 sm:gap-7 lg:grid-cols-[minmax(0,1.08fr)_minmax(340px,.92fr)]">
                <div className="relative overflow-hidden rounded-[26px] border border-white/10 bg-[linear-gradient(145deg,#38271b,#17130f)] p-4 shadow-[0_24px_70px_rgba(87,56,24,.25)] sm:rounded-[36px] sm:p-9 lg:sticky lg:top-5"><div className="absolute -left-16 -top-16 h-52 w-52 rounded-full bg-[#d6a441]/20 blur-3xl" /><div className="absolute -bottom-24 -right-20 h-64 w-64 rounded-full bg-[#8b5b2a]/20 blur-3xl" /><div className="relative mx-auto aspect-square max-w-[530px]"><div className="absolute left-1/2 top-[-3px] z-30 -translate-x-1/2"><div className="h-0 w-0 border-l-[14px] border-r-[14px] border-t-[30px] border-l-transparent border-r-transparent border-t-white drop-shadow-lg sm:border-l-[18px] sm:border-r-[18px] sm:border-t-[38px]" /></div><div className="absolute inset-1 rounded-full bg-[#8a5c2c] shadow-[0_0_0_5px_#e8c761,0_0_0_9px_#694019,0_18px_35px_rgba(0,0,0,.42)] sm:shadow-[0_0_0_7px_#e8c761,0_0_0_11px_#694019,0_20px_40px_rgba(0,0,0,.45)]" /><div className="absolute inset-[11px] overflow-hidden rounded-full transition-transform duration-[3400ms] ease-[cubic-bezier(.12,.67,.12,1)] sm:inset-4" style={{ transform: `rotate(${rotation}deg)`, background: `radial-gradient(circle at 48% 38%, rgba(255,255,255,.28), transparent 58%), ${wheelBackground}` }}><div className="absolute inset-2 rounded-full border border-white/65 shadow-[inset_0_0_32px_rgba(75,43,13,.12)] sm:inset-3" />{segments.map((segment, index) => {
                    const segmentAngle = 360 / segments.length;
                    return <div key={`divider-${segment.label}`} className="absolute left-1/2 top-1/2 z-[1] h-1/2 w-px origin-bottom bg-white/65 shadow-[0_0_2px_rgba(80,45,14,.25)]" style={{ transform: `translate(-50%, -100%) rotate(${index * segmentAngle}deg)` }} />;
                })}{segments.map((segment, index) => {
                    const segmentAngle = 360 / segments.length;
                    const angleDegrees = index * segmentAngle + segmentAngle / 2;
                    const angle = (angleDegrees * Math.PI) / 180;
                    const labelRotation = angleDegrees - 90;
                    return <div data-wheel-prize key={segment.label} className="absolute z-10 flex w-[31%] items-center justify-center text-center text-[clamp(9px,2.7vw,13px)] font-black leading-[1.08] tracking-tight text-[#25180f] [text-shadow:0_1px_0_rgba(255,255,255,.9),0_2px_5px_rgba(43,30,16,.28)] sm:text-[clamp(13px,1.65vw,17px)]" style={{ left: `${50 + Math.sin(angle) * 32.5}%`, top: `${50 - Math.cos(angle) * 32.5}%`, transform: `translate(-50%, -50%) rotate(${labelRotation}deg)` }}><span>{segment.label}</span></div>;
                })}</div><button onClick={spin} disabled={spinning || (!data?.campaign.adminTestMode && (!data?.campaign.active || !data?.account.availableSpins))} className="absolute left-1/2 top-1/2 z-20 grid h-[78px] w-[78px] -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border-[5px] border-white bg-[linear-gradient(145deg,#f7d26e,#c98028)] px-2 text-center text-[11px] font-black uppercase leading-tight text-[#2f2117] shadow-[0_10px_30px_rgba(0,0,0,.4),inset_0_2px_8px_rgba(255,255,255,.9),0_0_0_3px_rgba(111,65,20,.25)] transition hover:scale-105 disabled:cursor-not-allowed disabled:grayscale sm:h-28 sm:w-28 sm:border-[6px] sm:text-base"><span>{spinning ? 'ĐANG QUAY' : data?.campaign.spinButtonText}</span></button></div>{!data?.campaign.adminTestMode && <p className="relative mt-6 text-center text-sm font-semibold text-[#e9d9bd] sm:mt-8">Bạn còn <strong className="text-xl text-[#f6ca62]">{data?.account.availableSpins || 0}</strong> lượt quay</p>}</div>
