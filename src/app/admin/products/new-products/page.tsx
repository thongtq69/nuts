...
import { Sparkles, Search, Filter, Package, Plus, Star, TrendingUp, Eye, EyeOff } from 'lucide-react';
...
export default function NewProductsPage() {
    ...
    const handleAddToNew = async (productId: string) => {
        try {
            ...
            console.error('Error adding to new products:', error);
        } finally {
            setUpdating(null);
        }
    };

    const handleRemoveFromNew = async (productId: string) => {
        try {
            setUpdating(productId);
            const response = await fetch(`/api/products/${productId}`, {
                method: 'PATCH',
            ...
            console.error('Error removing from new products:', error);
        } finally {
            setUpdating(null);
        }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-1 ml-13">
                        Quản lý {products.length} sản phẩm mới trên trang chủ
                    </p>
                </div>
                <Link
                                ...
                                filteredProducts.map((product, index) => (
                                    <tr key={product._id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="text-center">
                                            <button
                                                onClick={() => handleRemoveFromNew(product._id)}
            ...
            <div className="glass-card">
                <div className="p-6 border-b border-slate-200">
                    ...
                    <p className="text-sm text-slate-600 mt-1">Chọn sản phẩm để thêm vào section "Sản phẩm mới"</p>
                </div>
                <div className="overflow-x-auto">
                    <table className="premium-table w-full">
                        <thead>
