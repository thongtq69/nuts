...
const getApiUrl = () => {
    ...
    // Server-side: use full URL (REQUIRED - relative URLs don't work server-side)

    // VERCEL_URL is automatically set by Vercel for previews and production
    if (process.env.VERCEL_URL) {
        return `https://${process.env.VERCEL_URL}/api`;
    }

    // Public version accessible in both client and server
    ...
    // Production fallback - use hardcoded domain
    // NOTE: VERCEL_URL should be available, but as safety net use known domain
    if (process.env.NODE_ENV === 'production') {
        return 'https://nuts-mocha-tau.vercel.app/api';
    }
